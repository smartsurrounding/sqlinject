...
@Stateless
@Local(PTHighlightQueryLocal.class)
public class PTHighlightQuery extends GenericDao<PTHighlightEntity> implements PTHighlightQueryLocal {
	...
	private ConditionResult getCondition(HighlightSearchForm form) {
		...
		if (form != null) {
			if (!StringUtil.isNullOrEmpty(form.getKeyword())) {
				conditions.add("(o.titleEn LIKE :keyword OR o.titleTc LIKE :keyword  OR o.titleSc LIKE :keyword)", "keyword", "%" + form.getKeyword() + "%");
			}
	...
	public Long count(HighlightSearchForm form) {
		String sql = "SELECT COUNT(o) FROM PTHighlightEntity o";
	...
	@Override
	public List<PTHighlightEntity> search(HighlightSearchForm form, Integer offset, Integer maxResult, List<SortMeta> sortList) {
		String sql = "SELECT o FROM PTHighlightEntity o";
		ConditionResult condition = getCondition(form);
		sql += condition.getWhereStatement();
